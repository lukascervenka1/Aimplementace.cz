...
/* OpenAI logo inline SVG — CDN slug unavailable on SimpleIcons */
const OpenAIIcon = () => (
  <svg className="tech-icon-svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
const techStack = [
  ...
  { slug: null,         color: null,     name: 'OpenAI',    icon: <OpenAIIcon /> },
      ...
      <div className="section-inner">
        ...
        <div className="tech-marquee-wrapper reveal reveal-delay-1">
          <div className="tech-marquee-track">
            {techStack.map((tech) => (
              <LogoItem tech={tech} key={`${tech.name}-1`} />
            ))}
            {techStack.map((tech) => (
              <LogoItem tech={tech} key={`${tech.name}-2`} />
            ))}
          </div>
        </div>
      </div>
